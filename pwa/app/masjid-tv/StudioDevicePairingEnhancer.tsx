...
export default function StudioDevicePairingEnhancer() {
  useEffect(() => {
    ...
    const makePairCard=(studio:HTMLElement)=>{
      ...
      card.innerHTML=`<div class="hdp-copy"><span class="hdp-kicker">DEVICE PAIRING</span><h3>Connect this display to Hassoun</h3><p>From any computer, phone, tablet, or Hassoun app, scan this QR or open Hassoun Display Pairing and enter the 6-digit code. Paired controllers can change this display remotely.</p><label class="hdp-name-label">Display name<input class="hdp-name" value="${device.name.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;")}" maxlength="40" aria-label="Display name" /></label><div class="hdp-code-label">PAIRING CODE</div><div class="hdp-code">${device.code}</div><div class="hdp-device">Device ID: ${device.id.slice(0,8).toUpperCase()}…${device.id.slice(-4).toUpperCase()}</div><div class="hdp-actions"><a class="hdp-manage" href="/masjid-tv/devices/">Manage connected displays</a><a class="hdp-website" href="/?mode=web">Website Mode</a></div></div><div class="hdp-qr-wrap"><img class="hdp-qr" src="${qrUrl}" alt="QR code to connect this display to Hassoun" /><span>Scan to connect</span></div>`;
      ...
      card.querySelector<HTMLAnchorElement>(".hdp-website")?.addEventListener("click",()=>{try{sessionStorage.setItem(WEB_SESSION_KEY,"1")}catch{}});
    ...
    const enhance=()=>{document.querySelector('[data-hassoun-website-mode="1"]')?.remove();const studio=findStudio();if(!studio)return;makeScrollable(studio);makePairCard(studio)};
