...
export default function PixelReplicaEnhancer() {
  useEffect(() => {
    const draw = () => {
      ...
      root.innerHTML = `
        <div class="px-brand-mask"></div><div class="px-brand-live">${logo ? `<img src="${esc(logo)}" alt="Masjid logo">` : ""}<div><h1>${esc(mosqueName)}</h1><p>${esc(location)}</p></div></div>
        <div class="px-clock-mask"></div><div class="px-clock-live"><button class="px-clock" type="button">${esc(clock)}</button><div class="px-dates"><span>${esc(dates[0] || "")}</span><i></i><span>${esc(dates[1] || "")}</span></div></div>
        <div class="px-verse-mask"></div><div class="px-verse-live">${esc(verse || "And establish prayer and give zakah and bow with those who bow.")}</div>
        ...
        <div class="px-prayer-values">${prayerRows.map((row, i) => `<div class="px-row px-row-${i}"><span class="px-pname">${esc(row.name)}</span><span class="px-adhan">${esc(row.adhan)}</span><span class="px-iqama">${esc(row.iqama)}</span></div>`).join("")}</div>
        <div class="px-ann-values">${(anns.length ? anns : [{title:"Announcements",body:"Add announcements from Masjid Display Studio."}]).map((a, i) => `<div class="px-ann px-ann-${i}"><strong>${esc(a.title)}</strong><span>${esc(a.body)}</span></div>`).join("")}</div>
