...
async function removeSolidBackground(src:string):Promise<string>{
 return await new Promise((resolve,reject)=>{const img=new Image();if(/^https?:/i.test(src))img.crossOrigin="anonymous";img.onerror=()=>reject(new Error("Logo could not be processed"));img.onload=()=>{const max=640,scale=Math.min(1,max/Math.max(img.width,img.height)),w=Math.max(1,Math.round(img.width*scale)),h=Math.max(1,Math.round(img.height*scale));const c=document.createElement("canvas");c.width=w;c.height=h;const x=c.getContext("2d",{willReadFrequently:true});if(!x)return reject(new Error("Image processing unavailable"));x.drawImage(img,0,0,w,h);try{const data=x.getImageData(0,0,w,h),p=data.data;const pts=[[0,0],[w-1,0],[0,h-1],[w-1,h-1],[Math.floor(w/2),0],[Math.floor(w/2),h-1]];let rr=0,gg=0,bb=0,n=0;for(const [px,py] of pts){const i=(py*w+px)*4;if(p[i+3]<20)continue;rr+=p[i];gg+=p[i+1];bb+=p[i+2];n++}if(!n)return resolve(c.toDataURL("image/png"));rr/=n;gg/=n;bb/=n;const hard=38,soft=82;for(let i=0;i<p.length;i+=4){const d=Math.sqrt((p[i]-rr)**2+(p[i+1]-gg)**2+(p[i+2]-bb)**2);if(d<=hard)p[i+3]=0;else if(d<soft)p[i+3]=Math.round(p[i+3]*((d-hard)/(soft-hard)))}x.putImageData(data,0,0);resolve(c.toDataURL("image/png"))}catch(e){reject(e)}};img.src=src})
}
 ...
 useEffect(()=>{
  ...
  const enhance=async()=>{
   ...
   if(identityHeading&&!identityHeading.parentElement?.querySelector("[data-logo-art-settings='1']")){
    ...
    box.querySelectorAll<HTMLButtonElement>("button").forEach(b=>b.onclick=async()=>{const mode=b.dataset.logoMode||"original";try{status.textContent=mode==="removed"?"Removing background…":"Saving…";const url=inputByLabel("logo url")?.value.trim()||String(remote.settings.logoUrl||"");let patch:Record<string,any>={logoBackgroundMode:mode};if(mode==="removed"&&url){patch.logoBackgroundRemovedUrl=await removeSolidBackground(url)}remote=await save(device,remote,patch);styleButtons();status.textContent=mode==="removed"?"Background-removed logo selected.":"Original logo selected.";window.dispatchEvent(new Event("hassoun-display-art-updated"))}catch(e){status.textContent=e instanceof Error?e.message:"Could not update logo"}});
   }
