 ...
 useEffect(()=>{
  ...
  const enhance=async()=>{
   ...
   const openMonth=(month:number,name:string,current:{name:string;settings:Record<string,any>},schedule:Record<string,any>)=>{
    closeModal();const year=new Date().getFullYear(),total=daysInMonth(year,month);
    ...
    modal.innerHTML=`<div style="display:flex;justify-content:space-between;align-items:center;padding:16px 18px;border-bottom:1px solid #35584f"><div><strong style="font-size:22px;color:#f1c86f">${name} ${year}</strong><div style="font-size:12px;color:#a8c2b9;margin-top:3px">Manual month editor</div></div><button data-close-month type="button" style="${btn}">Close</button></div><div data-month-warning style="padding:10px 18px"></div><div data-month-table style="overflow:auto;padding:0 18px 12px"></div><div style="display:flex;justify-content:flex-end;gap:10px;padding:14px 18px;border-top:1px solid #35584f"><button data-save-month type="button" style="${btn}">Save ${name}</button></div>`;
    ...
    warning.innerHTML=missing?`<span style="color:#ffd78b"><strong>Needs review:</strong> ${missing} blank prayer/Iqama values.</span>`:'<span style="color:#aee5bf"><strong>Healthy:</strong> all prayer and Iqama values are filled.</span>';
    wrap.appendChild(table);
    modal.querySelector<HTMLButtonElement>("[data-save-month]")!.onclick=async()=>{
      const next={...schedule};Array.from(tbody.querySelectorAll<HTMLTableRowElement>("tr")).forEach(tr=>{const vals=Array.from(tr.querySelectorAll<HTMLInputElement>("input")).map(i=>i.value.trim());next[tr.dataset.date!]={adhan:{fajr:vals[0],dhuhr:vals[2],asr:vals[4],maghrib:vals[6],isha:vals[8]},iqama:{fajr:vals[1],dhuhr:vals[3],asr:vals[5],maghrib:vals[7],isha:vals[9]}}});
      try{warning.textContent="Saving…";await save(device,{name:current.name,settings:{...current.settings,prayerSchedule:next,prayerMode:"custom"}});warning.innerHTML='<span style="color:#aee5bf"><strong>Saved.</strong></span>';window.dispatchEvent(new Event("hassoun-schedule-updated"));await render()}catch(e){warning.textContent=e instanceof Error?e.message:"Could not save month"}
    };
   };

   const render=async()=>{
    try{
     const current=await load(device);const schedule=(current.settings.prayerSchedule||{}) as Record<string,any>;const year=new Date().getFullYear();grid.innerHTML="";let empty=0,review=0;
     MONTHS.forEach((name,idx)=>{const month=idx+1,total=daysInMonth(year,month);let present=0,issues=0;for(let d=1;d<=total;d++){const r=schedule[key(year,month,d)];if(r)present++;issues+=rowHealth(r)}const healthy=present===total&&issues===0;if(!present)empty++;else if(!healthy)review++;const card=document.createElement("button");card.type="button";card.style.cssText=`text-align:left;padding:11px;border-radius:11px;border:1px solid ${healthy?'#4f9977':present?'#c89545':'#735e4c'};background:#0a3932;color:#fff;cursor:pointer`;card.innerHTML=`<strong style="font-size:15px">${name}</strong><div style="font-size:11px;margin-top:5px;color:${healthy?'#aee5bf':present?'#ffd78b':'#baa89b'}">${present}/${total} · ${healthy?'Ready':present?'Review':'Empty'}</div>`;card.onclick=()=>openMonth(month,name,current,schedule);grid.appendChild(card)});
     alerts.innerHTML=`<div style="padding:8px 10px;border-radius:9px;background:${empty||review?'#42321b':'#123d31'};color:${empty||review?'#ffd78b':'#aee5bf'}"><strong>Schedule:</strong> ${empty} empty · ${review} need review</div>`;
