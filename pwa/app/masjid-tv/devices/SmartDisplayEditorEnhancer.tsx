...
function findActive():Device|null{
  ...
  for(const article of articles){const txt=article.textContent||"";const m=txt.match(/\b\d{6}\b/);if(!m)continue;const manage=Array.from(article.querySelectorAll("button")).find(b=>(b.textContent||"").includes("Manage live"));if(manage&&article.getAttribute("style")?.includes("239, 198, 108"))return devices().find(d=>d.code===m[0])||null;}
  const heading=document.querySelector("h2");if(heading?.textContent?.includes("Live display editor")){const visible=articles.find(a=>a.getAttribute("style")?.includes("#efc66c"));const code=visible?.textContent?.match(/\b\d{6}\b/)?.[0];if(code)return devices().find(d=>d.code===code)||null;}
  return null;
}
...
export default function SmartDisplayEditorEnhancer(){
  useEffect(()=>{
    ...
    const clickCapture=(e:MouseEvent)=>{const t=e.target as HTMLElement|null;if(t?.tagName==="BUTTON"&&(t.textContent||"").includes("Manage live")){const article=t.closest("article");const code=article?.textContent?.match(/\b\d{6}\b/)?.[0];if(code)active=devices().find(d=>d.code===code)||null;}};
    ...
    const render=()=>{
      ...
      const h=headings.find(x=>["Mosque & display identity","Clock & date","Next prayer","Prayer table & Iqama","Announcements","Qur’an verse / reminder","Donation / website panel","Layout & appearance"].some(v=>(x.textContent||"").includes(v)));
      if(!h||!h.parentElement)return;
      const host=h.parentElement;const existing=host.querySelector<HTMLElement>("[data-smart-display-controls='1']");
      const key=(h.textContent||"").trim();if(existing?.dataset.section===key)return;existing?.remove();
      ...
      if(key.includes("Qur’an verse")){
        ...
        ai.onclick=async()=>{const topic=q.value.trim();if(topic.length<3){status.textContent="Describe the topic first";return}status.textContent="Hassoun AI is finding verses…";results.innerHTML="";try{const r=await fetch(`${API}/masjid-displays/ai/verse`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${active!.token}`},body:JSON.stringify({deviceId:active!.id,topic})});const d=await r.json() as {options?:Array<{text:string;reference:string}>;error?:string;source?:string};if(!r.ok)throw new Error(d.error||"AI request failed");status.textContent=d.source==="ai"?"AI suggestions ready":"Smart curated suggestions ready";(d.options||[]).forEach(opt=>{const b=btn(`${opt.reference} — ${opt.text}`);b.style.whiteSpace="normal";b.style.textAlign="left";b.onclick=()=>void run("Publishing verse…",async()=>{const value=`${opt.text} — ${opt.reference}`;await patch(active!,{verseMode:"manual",verseText:value,showVerse:true});setTextArea(value)});results.appendChild(b)})}catch(err){status.textContent=err instanceof Error?err.message:"AI request failed"}};
      }else if(key.includes("Clock")){const b=btn("Automatic local clock");b.onclick=()=>void run("Enabling automatic clock…",async()=>{await patch(active!,{clockMode:"local",showClock:true,showDate:true})});row.appendChild(b);
      }else if(key.includes("Next prayer")||key.includes("Prayer table")){const b=btn("Auto from prayer schedule");b.onclick=()=>void run("Using prayer schedule…",async()=>{await patch(active!,{prayerMode:"auto",showNextPrayer:true,showPrayerCards:true})});row.appendChild(b);
      }else if(key.includes("Announcements")){const b=btn("Auto rotate announcements");b.onclick=()=>void run("Enabling rotation…",async()=>{await patch(active!,{announcementMode:"rotate",showAnnouncements:true})});row.appendChild(b);
      }else if(key.includes("Donation")){const b=btn("Smart donation panel");b.onclick=()=>void run("Optimizing donation panel…",async()=>{await patch(active!,{showDonation:true,donationMode:"smart"})});row.appendChild(b);
      }else if(key.includes("Layout")){for(const [label,bg,card,accent] of [["Emerald & Gold","#063d34","#0a5548","#e7bd59"],["Midnight","#071b24","#102e3b","#d4b46b"],["Warm Mosque","#3a2d21","#57432e","#e1bf72"]]){const b=btn(label);b.onclick=()=>void run("Applying theme…",async()=>{await patch(active!,{backgroundColor:bg,cardColor:card,accentColor:accent})});row.appendChild(b)}}
      else {const b=btn("Keep this section synced automatically");b.onclick=()=>void run("Enabling smart sync…",async()=>{await patch(active!,{smartIdentity:true})});row.appendChild(b)}
      host.insertBefore(panel,h);
    };
    const timer=window.setInterval(render,700);render();
