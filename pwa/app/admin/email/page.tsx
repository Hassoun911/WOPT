...
type Profile = {
  ...
  sponsor_logo_present?: number; sponsor_logo_mime?: string | null;
};
...
function logoPreviewUrl(profile: Profile) {
  return `${API}/email/sponsor-logo/${encodeURIComponent(profile.template_key)}?v=${Date.now()}`;
}

async function imageToEmailLogo(file: File) {
  if (!/^image\/(png|jpeg)$/.test(file.type)) throw new Error("Please choose a PNG or JPG sponsor logo.");
  const objectUrl = URL.createObjectURL(file);
  try {
    const image = await new Promise<HTMLImageElement>((resolve, reject) => {
      const img = new Image();
      img.onload = () => resolve(img);
      img.onerror = () => reject(new Error("Unable to read sponsor logo."));
      img.src = objectUrl;
    });
    const maxW = 720; const maxH = 320;
    const scale = Math.min(1, maxW / image.naturalWidth, maxH / image.naturalHeight);
    const canvas = document.createElement("canvas");
    canvas.width = Math.max(1, Math.round(image.naturalWidth * scale));
    canvas.height = Math.max(1, Math.round(image.naturalHeight * scale));
    const context = canvas.getContext("2d");
    if (!context) throw new Error("Unable to prepare sponsor logo.");
    context.drawImage(image, 0, 0, canvas.width, canvas.height);
    let dataUrl = canvas.toDataURL(file.type === "image/jpeg" ? "image/jpeg" : "image/png", 0.9);
    if (dataUrl.length > 465_000) dataUrl = canvas.toDataURL("image/jpeg", 0.82);
    if (dataUrl.length > 465_000) throw new Error("Sponsor logo is still too large. Please choose a smaller image.");
    return dataUrl;
  } finally {
...
export default function AdminEmailPage() {
  ...
  const [logoClears, setLogoClears] = useState<Record<string, boolean>>({});
  ...
  async function chooseSponsorLogo(profile: Profile, file?: File) {
    if (!file) return;
    try {
      const dataUrl = await imageToEmailLogo(file);
      setLogoDrafts((current) => ({ ...current, [profile.template_key]: dataUrl }));
      setLogoClears((current) => ({ ...current, [profile.template_key]: false }));
      setError("");
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to prepare sponsor logo."); }
  }
  ...
  async function saveSponsor(profile: Profile) {
    ...
    await postAction({
      action: "update_template_profile", templateKey: profile.template_key, sponsorName, sponsorUrl, sponsorMessageEn, sponsorMessageAr,
      ...(logoDrafts[profile.template_key] ? { sponsorLogoDataUrl: logoDrafts[profile.template_key] } : {}),
      ...(logoClears[profile.template_key] ? { clearSponsorLogo: true } : {})
    });
    setLogoDrafts((current) => { const next = { ...current }; delete next[profile.template_key]; return next; });
    setLogoClears((current) => { const next = { ...current }; delete next[profile.template_key]; return next; });
  }
  ...
  async function schedule(event: FormEvent) {
    ...
    try {
      ...
      await api("/admin/email/campaigns", { method: "POST", body: JSON.stringify({
        ...
        textEn: messageEn, subjectAr: subjectAr || undefined,
  ...
  return <main style={s.page}>
    ...
    {tab === "templates" ? <section style={s.wrap}>
      <div style={s.info}><strong>Template controls</strong><span>Qur’an Tahfiz School and the unsubscribe/manage link are automatically added to subscriber emails. Sponsor logo, name, message and link are controlled here.</span></div>
      <div style={s.cards}>{data.profiles.map(profile => {
        const draft = logoDrafts[profile.template_key];
        const cleared = logoClears[profile.template_key];
        const showSaved = profile.sponsor_logo_present === 1 && !cleared;
        return <article key={profile.template_key} style={s.card}>
          ...
          <div style={s.sponsorBox}>
            ...
            <div style={s.logoRow}>
              <div style={s.logoPreview}>{draft ? <img src={draft} alt="New sponsor logo preview" style={s.logoImg}/> : showSaved ? <img src={logoPreviewUrl(profile)} alt="Saved sponsor logo" style={s.logoImg}/> : <span style={s.logoEmpty}>No logo</span>}</div>
              <div style={{display:"grid",gap:7,alignContent:"center"}}><label style={s.uploadButton}>Upload sponsor logo<input type="file" accept="image/png,image/jpeg" style={{display:"none"}} onChange={(event)=>void chooseSponsorLogo(profile,event.target.files?.[0])}/></label>{draft||showSaved?<button type="button" style={s.dangerButton} onClick={()=>{setLogoDrafts(current=>{const next={...current};delete next[profile.template_key];return next;});setLogoClears(current=>({...current,[profile.template_key]:true}));}}>Remove logo</button>:null}<span style={s.small}>PNG/JPG. Optimized automatically for email.</span></div>
            </div>
            <div style={s.two}><input id={`sponsor-name-${profile.template_key}`} defaultValue={profile.sponsor_name || ""} placeholder="Sponsor name" style={s.input}/><input id={`sponsor-url-${profile.template_key}`} defaultValue={profile.sponsor_url || ""} placeholder="Sponsor website / link" style={s.input}/></div>
            <input id={`sponsor-message-${profile.template_key}`} defaultValue={profile.sponsor_message_en || ""} placeholder="Sponsor message (English)" style={s.input}/>
            <input id={`sponsor-message-ar-${profile.template_key}`} defaultValue={profile.sponsor_message_ar || ""} placeholder="رسالة الراعي (العربية)" dir="rtl" style={s.input}/>
            <button disabled={busy} onClick={()=>void saveSponsor(profile)} style={s.secondaryButton}>Save sponsor details</button>
          </div>
        </article>;
      })}</div>
