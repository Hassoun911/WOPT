...
export default function QuranHeaderSearchEnhancer() {
  ...
  useEffect(() => {
    ...
    style.textContent = `
      ...
      .wopt-search-backdrop{position:fixed;z-index:4000;inset:0;display:none;background:rgba(0,0,0,.38);align-items:flex-start;justify-content:center;padding:calc(env(safe-area-inset-top) + 74px) 12px 18px}.wopt-search-backdrop.open{display:flex}
      ...
      .mushaf-ayah.wopt-search-hit{background:#fff3a8!important;box-shadow:0 0 0 6px rgba(237,184,36,.26)!important;border-radius:8px!important;transition:background .25s ease,box-shadow .25s ease}.quran-word.wopt-search-word{background:#efcf60!important;color:#111!important;border-radius:5px!important;box-shadow:0 0 0 3px rgba(239,207,96,.26)!important}
    ...
    overlay.innerHTML = `
      <section class="wopt-search-sheet" role="dialog" aria-modal="true" aria-label="Search Qur’an">
        ...
        <div class="wopt-search-help">Search Arabic or English, paste Qur’an text, or enter a verse like 2:255. Tap a result to open and highlight it.</div>
    ...
    const renderHistory = () => {
      ...
      chips.innerHTML = history.map((item) => `<button class="wopt-search-chip" type="button" data-history="${encodeURIComponent(item)}">${escapeHtml(item)}</button>`).join("");
    };
    ...
    const render = (items: SearchResult[]) => {
      ...
      results.innerHTML = items.slice(0, 40).map((item) => {
        ...
        return `<button class="wopt-search-result" type="button" data-key="${escapeHtml(key)}"><span class="wopt-search-key">${escapeHtml(key)}</span><div class="wopt-search-ar">${escapeHtml(arabic)}</div>${english ? `<div class="wopt-search-en">${escapeHtml(english)}</div>` : ""}</button>`;
      }).join("");
    };
    ...
    const applyPendingHighlight = () => {
      ...
      if (q && !/^\d{1,3}\s*[:\/]\s*\d{1,3}$/.test(target.query)) {
        ...
        ayah.querySelectorAll<HTMLElement>(".quran-word").forEach((word) => {
          ...
          if (text && terms.some((term) => text.includes(term) || term.includes(text))) word.classList.add("wopt-search-word");
        });
      }

      ayah.scrollIntoView({ behavior: "smooth", block: "center" });
      window.localStorage.removeItem(SEARCH_TARGET_KEY);
      if (window.location.search) window.history.replaceState({}, "", pathname);
    ...
    const highlightTimer = window.setInterval(() => {
      highlightAttempts += 1;
      if (applyPendingHighlight() || highlightAttempts > 60) window.clearInterval(highlightTimer);
    }, 300);
