...
const STORAGE_KEY = "wopt-quran-reader-style-v2";
...
function loadSettings(): ReaderStyleSettings {
  try {
    ...
    return {
      ...
      fontSize: Math.max(26, Math.min(72, Number(parsed.fontSize) || DEFAULTS.fontSize)),
      lineHeight: Math.max(1.25, Math.min(2.8, Number(parsed.lineHeight) || DEFAULTS.lineHeight)),
...
export default function QuranReaderStyleEnhancer() {
  ...
  useEffect(() => {
    ...
    style.textContent = `
      ...
      .quran-app.wopt-layout-book .mushaf-shell{max-width:880px!important;border-radius:8px!important;padding:0 26px 28px!important;box-shadow:0 12px 45px rgba(40,36,22,.10)!important}
      .quran-app.wopt-layout-book .mushaf-text{padding:34px 10px 46px!important;text-align:justify!important;text-align-last:auto!important;column-gap:0!important}
      .quran-app.wopt-layout-book .mushaf-ayah{display:inline!important}.quran-app.wopt-layout-book .inline-translation,.quran-app.wopt-layout-book .inline-transliteration{display:block!important}
    ...
    const layoutLabel: Record<ReaderLayout, [string, string]> = {
      book: ["Book", "Mushaf-style continuous reading"],
    ...
    const renderPanelBody = () => {
      ...
      body.innerHTML = `
        ...
        <section class="wopt-style-section"><strong>Arabic typography</strong>
          ...
          <div class="wopt-style-row"><label>Text size</label><div class="wopt-size-control"><button type="button" data-size-step="-2">−</button><input data-style="fontSize" type="range" min="26" max="72" step="1" value="${settings.fontSize}"><button type="button" data-size-step="2">+</button><output>${settings.fontSize}px</output></div></div>
          <div class="wopt-style-row"><label>Line spacing</label><div class="wopt-size-control"><button type="button" data-line-step="-0.1">−</button><input data-style="lineHeight" type="range" min="1.25" max="2.8" step="0.05" value="${settings.lineHeight}"><button type="button" data-line-step="0.1">+</button><output>${settings.lineHeight.toFixed(2)}</output></div></div>
        </section>
    ...
    const openPanel = () => {
      ...
      backdrop.addEventListener("click", (event) => {
        ...
        if (step) { set({ fontSize: Math.max(26, Math.min(72, settings.fontSize + Number(step))) }); return; }
        const lineStep = target.closest<HTMLElement>("[data-line-step]")?.dataset.lineStep;
        if (lineStep) { set({ lineHeight: Math.max(1.25, Math.min(2.8, Number((settings.lineHeight + Number(lineStep)).toFixed(2)))) }); return; }
        ...
        if (preset === "classic") set({ textColor: "#102f28", pageColor: "#fffdf6" });
        if (preset === "cream") set({ textColor: "#2b2a21", pageColor: "#fbf1d8" });
        if (preset === "green") set({ textColor: "#123d32", pageColor: "#f1f8f3" });
