...
s = re.sub(r'version:\s*(?:process\.env\.EXPO_APP_VERSION\s*\|\|\s*)?"[^"]+"', 'version: "1.0.41"', s, count=1)
s = re.sub(r'versionCode:\s*\d+', 'versionCode: 85', s, count=1)
if 'version: "1.0.41"' not in s or 'versionCode: 85' not in s:
    raise SystemExit('Unable to bump Android v1.0.41 / versionCode 85')
...
for marker in ['locationRequiredFallback','shouldUseOfficialWindsor','preferences.locationMode === "mosque"','return refreshed']:
    if marker not in prayer: raise SystemExit(f'Missing prayer marker: {marker}')
if 'windsorFallback()' in prayer:
    raise SystemExit('Unsafe Windsor fallback survived v1.0.41')
for marker in ['Mosques Near Me','mosquesNearMeCard','setMosquesOpen(true)']:
    if marker not in page: raise SystemExit(f'Missing Mosques Near Me navigation marker: {marker}')
for marker in ['overpass-api.de/api/interpreter','overpass.kumi.systems/api/interpreter','MOSQUE_DIRECTORY_UNAVAILABLE','export async function findNearbyMosques','Show nearest mosque on Home','<Switch']:
    if marker not in mosques: raise SystemExit(f'Missing mosque marker: {marker}')
for marker in ['nearestMosqueEnabled','NEAREST MOSQUE','mosqueDistanceLabel','findNearbyMosques(context.location.latitude','nearestMosqueCard']:
    if marker not in home: raise SystemExit(f'Missing Home nearest-mosque marker: {marker}')
for marker in ['SHOW_NEAREST_MOSQUE_HOME_KEY','saveShowNearestMosqueOnHome','subscribeShowNearestMosqueOnHome']:
    if marker not in pref: raise SystemExit(f'Missing nearest preference marker: {marker}')
for marker in ['Location.hasServicesEnabledAsync()','Notifications.getPermissionsAsync()','PermissionsAndroid.PERMISSIONS.CAMERA','PermissionsAndroid.PERMISSIONS.RECORD_AUDIO']:
    if marker not in permissions: raise SystemExit(f'Missing permission marker: {marker}')
for marker in ['version: "1.0.41"','versionCode: 85','android.permission.ACCESS_FINE_LOCATION']:
    if marker not in cfg: raise SystemExit(f'Missing config marker: {marker}')
print('HASSOUN_ANDROID_V1041_SOURCE_VERIFIED')
PY
...
test -s "$APK"
OUT="mobile/android/app/build/outputs/apk/release/Hassoun-v1.0.41-nearest-mosque-home.apk"
cp "$APK" "$OUT"
sha256sum "$OUT" | tee "$OUT.sha256"
echo "HASSOUN_V1041_APK_READY=$OUT"
