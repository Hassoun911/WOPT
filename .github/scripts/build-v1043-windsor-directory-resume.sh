...
python3 .github/scripts/fix-v1043-windsor-directory-and-nested-resume.py
...
s = re.sub(r'version:\s*(?:process\.env\.EXPO_APP_VERSION\s*\|\|\s*)?"[^"]+"', 'version: "1.0.43"', s, count=1)
s = re.sub(r'versionCode:\s*\d+', 'versionCode: 87', s, count=1)
if 'version: "1.0.43"' not in s or 'versionCode: 87' not in s:
    raise SystemExit('Unable to bump Android v1.0.43 / versionCode 87')
...
calc = Path('mobile/src/PrayerCalculationSettingsPage.tsx').read_text(encoding='utf-8')
...
for marker in ['fetchWithTimeout','MOSQUE_SEARCH_TIMEOUT','WINDSOR_TRUSTED_MOSQUES','return findWindsorMosques(latitude, longitude);']:
    if marker not in mosques: raise SystemExit(f'Missing v1.0.43 mosque marker: {marker}')
for name in ['Windsor Mosque','Windsor Islamic Association Centre','Tarbiyah Centre','West Musallah Windsor','Rose City Islamic Centre','Masjid At-Taqwa','Masjid Noor-ul-Islam','ALBER Mosque','Al-Hijra Academy']:
    if name not in mosques: raise SystemExit(f'Missing trusted Windsor mosque: {name}')
for marker in ['nearestMosqueEnabled','NEAREST MOSQUE','nearestMosqueCard']:
    if marker not in home: raise SystemExit(f'Missing Home nearest-mosque marker: {marker}')
for marker in ['HASSOUN_SETTINGS_NESTED_RESUME_V1','hassoun:settings-nested-route:v1','settingsRouteReady']:
    if marker not in hub: raise SystemExit(f'Missing SettingsHub resume marker: {marker}')
for marker in ['HASSOUN_MOSQUES_CHILD_RESUME_V1','hassoun:prayer-calc-child-route:v1','mosquesRouteReady']:
    if marker not in calc: raise SystemExit(f'Missing Prayer Calculation child resume marker: {marker}')
...
for marker in ['HASSOUN_BACKGROUND_RESUME_NO_RESET_V4','hassoun:resume-exact-screen:v1']:
    if marker not in app: raise SystemExit(f'Missing top-level background resume marker: {marker}')
for marker in ['version: "1.0.43"','versionCode: 87']:
    if marker not in cfg: raise SystemExit(f'Missing config marker: {marker}')
print('HASSOUN_ANDROID_V1043_SOURCE_VERIFIED')
PY
...
print('HASSOUN_ANDROID_V1043_MANIFEST_VERIFIED')
PY
...
test -s "$APK"
OUT="mobile/android/app/build/outputs/apk/release/Hassoun-v1.0.43-windsor-mosques-resume.apk"
cp "$APK" "$OUT"
sha256sum "$OUT" | tee "$OUT.sha256"
echo "HASSOUN_V1043_APK_READY=$OUT"
