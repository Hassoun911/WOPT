...
# Local quick controls. Earlier reconstruction scripts can format the array differently,
# so modify individual entries instead of depending on one exact serialized row.
page = page.replace('["clock24Hour",t("Use 24-hour clock","استخدام نظام 24 ساعة")]', '["clock24Hour",t("24-hour clock","نظام 24 ساعة")]', 1)
page = page.replace('["clock24Hour",t("24-hour clock","نظام 24 ساعة")]', '["clock24Hour",t("24-hour clock","نظام 24 ساعة")]', 1)
# Remove the obsolete Clock AM/PM item with an optional preceding/following comma.
page = re.sub(r',?\["showClockPeriod",t\("Clock AM / PM","AM / PM للساعة"\)\]', '', page, count=1)
# If clock24Hour was never inserted by the older compat patch, add it immediately after seconds.
if '["clock24Hour",t("24-hour clock","نظام 24 ساعة")]' not in page:
    seconds_item = '["showSeconds",t("Show seconds","إظهار الثواني")]'
    if seconds_item in page:
        page = page.replace(seconds_item, seconds_item + ',["clock24Hour",t("24-hour clock","نظام 24 ساعة")]', 1)
if '["clock24Hour",t("24-hour clock","نظام 24 ساعة")]' not in page:
...
# There must be no active local Clock AM/PM quick-control item left.
