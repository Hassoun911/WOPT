...
'        const result = await schedulePrayerNotifications(loaded.prayerTimes, chosenLocale, savedPhoneAlertPreferences, { locationLabel: loaded.location.label, timeZone: loaded.location.timezone });'
)
rep(
...
# Current-location context for every prayer notification reschedule.
rep(
'      const result = await schedulePrayerNotifications(prayerTimes, nextLocale, phoneAlertPreferences);',
'      const result = await schedulePrayerNotifications(prayerTimes, nextLocale, phoneAlertPreferences, { locationLabel: prayerLocation.label, timeZone: prayerLocation.timezone });'
)
rep(
'      const result = await schedulePrayerNotifications(prayerTimes, locale, preferences);',
'      const result = await schedulePrayerNotifications(prayerTimes, locale, preferences, { locationLabel: prayerLocation.label, timeZone: prayerLocation.timezone });'
)
rep(
'      const result = await schedulePrayerNotifications(prayerTimes, locale, nextPreferences);',
'      const result = await schedulePrayerNotifications(prayerTimes, locale, nextPreferences, { locationLabel: prayerLocation.label, timeZone: prayerLocation.timezone });'
)

# Keep Islamic event scheduling in the same detected timezone for current state operations.
...
'<View style={styles.brandText}><Text style={styles.title}>Hassoun</Text><Text numberOfLines={1} style={styles.subtitle}>📍 {prayerLocation.label} • {locale === "ar" ? "مواقيت الصلاة" : "Prayer Times"}</Text></View>'
)

# Make the schedule source visible and truthful without changing card/layout structure.
...
'<Text style={styles.footer}>{prayerLocation.source === "windsor_islamic_association" ? "Official Windsor Islamic Association schedule" : prayerLocation.source === "aladhan" ? `Local prayer calculation • ${prayerLocation.label}` : `Saved prayer schedule • ${prayerLocation.label}`} • {prayerLocation.timezone}</Text>'
)

APP.write_text(s, encoding="utf-8")

# New installable test build; code 65 stays above the store generation codes already used.
...
# Guardrails: UI baseline stays intact; only location/version behavior was altered.
checks = [
    'AboutHassounPage',
    'hassoun-logo.png',
    '📍 {prayerLocation.label}',
    'loadPrayerTimes().then(async (refreshed)',
