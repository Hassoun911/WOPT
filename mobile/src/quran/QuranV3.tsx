...
export default function QuranV3({ locale, onBackHome, onAppNavVisibilityChange }: Props) {
  ...
  const radioScreen = (
    <ScrollView style={styles.flex} contentContainerStyle={styles.radioContent} showsVerticalScrollIndicator={false}>
      {topBar(tr("Qur’an Radio", "إذاعة القرآن"), tr("Build your own continuous Qur’an listening", "استماع متواصل بالطريقة التي تختارها"))}
      <View style={styles.radioHero}><View style={styles.radioHeroIcon}><Text style={styles.radioHeroEmoji}>📻</Text></View><View style={styles.topCopy}><Text style={[styles.radioHeroTitle, ar && styles.rtl]}>{tr("Your Qur’an station", "إذاعتك القرآنية")}</Text><Text style={[styles.radioHeroText, ar && styles.rtl]}>{tr("Choose a reciter, a Surah, a playlist, or play the Qur’an continuously from any point.", "اختر القارئ والسورة أو أنشئ قائمة تشغيل أو شغّل القرآن باستمرار من أي موضع.")}</Text></View></View>

      <Text style={styles.radioSectionLabel}>{tr("RECITER", "القارئ")}</Text>
      ...
      <View style={styles.radioCard}><View style={styles.radioCardHeader}><View style={styles.topCopy}><Text style={styles.radioCardTitle}>🎶 {tr("My playlist", "قائمة التشغيل")}</Text><Text style={styles.radioCardHint}>{tr("Surahs play in the order shown below.", "تعمل السور بالترتيب الظاهر أدناه.")}</Text></View>{radioPlaylist.length ? <Pressable onPress={() => persistPlaylist([])} style={styles.clearPill}><Text style={styles.clearPillText}>{tr("Clear", "مسح")}</Text></Pressable> : null}</View>{radioPlaylist.length ? <View style={styles.playlistWrap}>{radioPlaylist.map((surahNumber, index) => { const surah = getSurah(surahNumber); return <View key={`${surahNumber}-${index}`} style={styles.playlistItem}><View style={styles.playlistNumber}><Text style={styles.playlistNumberText}>{index + 1}</Text></View><View style={styles.topCopy}><Text style={styles.playlistTitle}>{ar ? surah?.nameArabic : surah?.nameTransliterated}</Text><Text style={styles.playlistMeta}>{tr(`Surah ${surahNumber}`, `سورة ${num(surahNumber)}`)}</Text></View><Pressable onPress={() => persistPlaylist(radioPlaylist.filter((_item, itemIndex) => itemIndex !== index))} style={styles.removePlaylist}><Text style={styles.removePlaylistText}>×</Text></Pressable></View>; })}</View> : <View style={styles.emptyPlaylist}><Text style={styles.emptyPlaylistText}>{tr("Add Surahs above to create a playlist.", "أضف سوراً أعلاه لإنشاء قائمة تشغيل.")}</Text></View>}<View style={styles.twoActionRow}><Pressable disabled={!radioPlaylist.length} onPress={() => playPlaylist(false)} style={[styles.radioPrimary, !radioPlaylist.length && styles.disabled]}><Text style={styles.radioPrimaryText}>▶️ {tr("Play playlist", "تشغيل القائمة")}</Text></Pressable><Pressable disabled={!radioPlaylist.length} onPress={() => playPlaylist(true)} style={[styles.radioSecondary, !radioPlaylist.length && styles.disabled]}><Text style={styles.radioSecondaryText}>🔁 {tr("Loop playlist", "تكرار القائمة")}</Text></Pressable></View></View>

      <View style={styles.radioCard}><Text style={styles.radioCardTitle}>🌙 {tr("Continuous Qur’an", "القرآن المتواصل")}</Text><Text style={styles.radioCardHint}>{tr("Choose where to begin. Continue to the end, or choose a Surah where playback should stop.", "اختر من أين تبدأ. استمر إلى آخر القرآن أو اختر السورة التي تريد التوقف عندها.")}</Text><Text style={styles.radioFieldLabel}>{tr("Start at", "ابدأ من")}</Text>{surahStepper(radioStartSurah, (next) => { setRadioStartSurah(next); if (radioEndSurah < next) setRadioEndSurah(next); })}<View style={styles.ongoingRow}><View style={styles.topCopy}><Text style={styles.ongoingTitle}>{tr("Continue to the end of the Qur’an", "الاستمرار إلى آخر القرآن")}</Text><Text style={styles.ongoingText}>{tr("Turn this off to choose a stopping Surah.", "أوقف هذا الخيار لتحديد سورة التوقف.")}</Text></View><Switch value={radioOngoing} onValueChange={setRadioOngoing} /></View>{!radioOngoing ? <><Text style={styles.radioFieldLabel}>{tr("Stop after", "توقف بعد")}</Text>{surahStepper(radioEndSurah, (next) => setRadioEndSurah(Math.max(radioStartSurah, next)))}</> : null}<View style={styles.twoActionRow}><Pressable onPress={() => playFullQuranRange(false)} style={styles.radioPrimary}><Text style={styles.radioPrimaryText}>▶️ {tr("Start listening", "ابدأ الاستماع")}</Text></Pressable><Pressable onPress={() => playFullQuranRange(true)} style={styles.radioSecondary}><Text style={styles.radioSecondaryText}>🔁 {tr("Loop range", "تكرار النطاق")}</Text></Pressable></View></View>
    </ScrollView>
  );
