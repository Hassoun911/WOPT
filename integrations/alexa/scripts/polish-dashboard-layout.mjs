...
const frame = (left, top, width, height, color, radius = 22, extra = {}) => ({
  ...
  borderRadius: `${radius}dp`, backgroundColor: color, ...extra,
});
...
for (const key of ['fajr', 'dhuhr', 'asr', 'maghrib', 'isha']) {
  ...
  tileItems.push({
    type: 'Frame', position: 'absolute', left: `${left}dp`, top: '554dp', width: '218dp', height: '145dp', borderRadius: '22dp',
    backgroundColor: `\${hassounData.nextPrayer.prayer == '${key}' ? '#168878' : '#FFFDF8'}`,
    item: {
      type: 'Container', width: '100%', height: '100%', paddingLeft: '22dp', paddingRight: '18dp', paddingTop: '16dp', paddingBottom: '14dp',
      items: [
        { type: 'Text', text: english[key], fontSize: '23dp', fontWeight: 700, color: `\${hassounData.nextPrayer.prayer == '${key}' ? '#FFFFFF' : '#0D4550'}` },
        { type: 'Text', text: arabic[key], fontSize: '21dp', fontWeight: 700, color: `\${hassounData.nextPrayer.prayer == '${key}' ? '#FFFFFF' : '#0D4550'}`, paddingTop: '2dp' },
        { type: 'Text', text: `\${hassounData.prayers.${key}.displayTime}`, fontSize: '26dp', fontWeight: 700, color: `\${hassounData.nextPrayer.prayer == '${key}' ? '#FFFFFF' : '#0D4550'}`, paddingTop: '6dp' }
      ]
    }
  });
...
const apl = {
  ...
  mainTemplate: {
    ...
    items: [{
      type: 'Frame', width: '100vw', height: '100vh', backgroundColor: '#F7F2E8',
      item: {
        type: 'Container', width: '1280dp', height: '800dp',
        items: [
          ...
          T('NEXT PRAYER', 104, 171, 260, 16, '#EFF7F3', 700, { letterSpacing: 3 }),
          ...
          T('TODAY', 810, 177, 190, 15, '#5B6665', 700, { letterSpacing: 3 }),
          ...
          T('NEXT ISLAMIC EVENT', 810, 352, 270, 15, '#5B6665', 700, { letterSpacing: 3 }),
          ...
          T('—  FAITH   |   FAMILY   |   COMMUNITY', 871, 742, 350, 15, '#9AA2A1', 500, { textAlign: 'right', letterSpacing: 2 })
        ]
      }
    }]
  }
...
console.log('Applied approved 1280x800 Alexa dashboard as sharp native APL.');
