  ...
  mainTemplate:{parameters:['hassounData'],items:[{
    type:'Frame',width:'100vw',height:'100vh',backgroundColor:'#F7F2E8',item:{
      type:'Container',width:'960dp',height:'600dp',items:[
        F(40,32,48,48,'#1AA896',24),T('H',40,40,48,28,'#FFFFFF',700,{textAlign:'center'}),T('HASSOUN',104,35,250,30,'#0D6B66',700),T('Prayer Dashboard',104,69,220,16,'#9AA7A4',500),T('⌖',612,44,20,16,'#0D6B66',700,{textAlign:'center'}),T('${hassounData.location}',640,39,190,18,'#0C555E',700),T('${hassounData.dateLabel}',640,66,210,13,'#9AA7A4',500),T('${hassounData.hijriDate}',640,87,210,13,'#10BCA8',600),T('☾',860,34,42,30,'#F1D74F',700,{textAlign:'center'}),T('${hassounData.weatherTemp}',842,78,76,14,'#647876',700,{textAlign:'center',when:"${hassounData.weatherTemp != ''}"}),T('${hassounData.weatherLabel}',838,96,84,11,'#9AA7A4',500,{textAlign:'center',when:"${hassounData.weatherLabel != ''}"}),F(40,116,520,272,'#18A392',22),T('NEXT PRAYER',76,148,180,13,'#F4FFFD',700,{letterSpacing:2}),T('☾',500,146,28,24,'#FAEA73',700,{textAlign:'center'}),T('${hassounData.nextPrayer.name}',76,188,300,43,'#FFFFFF',700),T('${hassounData.nextPrayer.arabicName}',76,247,300,27,'#FFFFFF',700),T('${hassounData.nextPrayer.displayTime}',76,291,250,27,'#FFFFFF',700),F(64,332,300,46,'#118575',23),T('◷',82,342,28,21,'#F5D83C',700,{textAlign:'center'}),T('${hassounData.nextPrayer.timeUntil}',120,338,205,17,'#FFFFFF',700),T('until Adhan',120,359,170,12,'#FFFFFF',500),F(397,234,82,104,'#FFFFFF',16,{opacity:0.12}),F(368,272,22,66,'#FFFFFF',11,{opacity:0.12}),F(486,272,22,66,'#FFFFFF',11,{opacity:0.12}),F(426,207,24,30,'#FFFFFF',12,{opacity:0.12}),F(423,193,30,18,'#FFFFFF',15,{opacity:0.12}),F(426,300,24,38,'#0D7A6D',12,{opacity:0.25}),F(376,258,7,14,'#FFFFFF',4,{opacity:0.18}),F(494,258,7,14,'#FFFFFF',4,{opacity:0.18}),F(590,132,330,112,'#FFFDF8',18,{borderWidth:'1dp',borderColor:'#E7DED2'}),T('TODAY',612,151,130,12,'#8A9693',700,{letterSpacing:2}),T('${hassounData.dateLabel}',612,180,245,18,'#0D5960',700),T('${hassounData.hijriDate}',612,209,245,14,'#12B6A2',500),T('▣',868,151,28,22,'#C5D1CE',700,{textAlign:'center'}),F(590,262,330,112,'#FFFDF8',18,{borderWidth:'1dp',borderColor:'#E7DED2'}),T('NEXT ISLAMIC EVENT',612,281,230,12,'#8A9693',700,{letterSpacing:2}),T('${hassounData.eventName}',612,311,230,19,'#0D5960',700),T('${hassounData.eventWhen}',612,341,210,14,'#7B8785',500),T('♜',866,283,28,24,'#E0B532',700,{textAlign:'center'}),...prayerItems,T('Closer to what matters.',40,556,300,20,'#0D8A7B',400,{fontStyle:'italic'}),T('—  FAITH  |  FAMILY  |  COMMUNITY',650,562,270,10,'#C4CECB',500,{textAlign:'right',letterSpacing:1.2})]}}]}};
