...
const contentCard = String.raw`function contentCard(item: ContentRow, _locale: Locale, accent: string) {
  ...
  return '<tr><td align="center" style="padding:0 22px 14px;text-align:center"><table role="presentation" width="100%" style="background:#fffdf8;border:1px solid #e4ddcf;border-radius:16px;margin:0 auto;text-align:center"><tr><td align="center" style="padding:14px;text-align:center">'
    ...
    + '<div dir="rtl" style="text-align:center;margin-top:8px"><div style="font-size:11px;color:' + accent + ';font-weight:900">' + escapeHtml(titleAr) + '</div><div style="font-size:14px;line-height:1.75;color:#31564c;font-weight:700;margin-top:4px">' + escapeHtml(bodyAr) + '</div></div>'
    + (item.source_ref ? '<div style="font-size:10px;color:#89938f;margin-top:7px;text-align:center">' + escapeHtml(item.source_ref) + '</div>' : '')
    + '</td></tr></table></td></tr>';
}`;
...
const enhancementHtml = String.raw`function enhancementHtml(profile: ProfileRow, content: Map<string, ContentRow>, data: Record<string, unknown>, _locale: Locale) {
  ...
  if (event && (profile.include_islamic_occasion === 1 || profile.include_occasion_countdown === 1)) {
    ...
    blocks.push('<tr><td align="center" style="padding:0 22px 14px;text-align:center"><table role="presentation" width="100%" style="background:#fff3cc;border:1px solid #e1c56f;border-radius:16px;margin:0 auto;text-align:center"><tr><td align="center" style="padding:14px;text-align:center">'
      + '<div dir="ltr" style="text-align:center"><div style="font-size:10px;letter-spacing:1.2px;color:#9a772c;font-weight:900">UPCOMING ISLAMIC OCCASION</div><div style="font-size:19px;font-weight:900;color:#173f35;margin-top:5px">' + escapeHtml(event.emoji || "🌙") + ' ' + escapeHtml(nameEn) + '</div><div style="font-size:13px;line-height:1.5;color:#60716b;margin-top:5px">' + escapeHtml(descriptionEn) + '</div></div>'
      + '<div dir="rtl" style="text-align:center;margin-top:8px"><div style="font-size:11px;color:#9a772c;font-weight:900">المناسبة الإسلامية القادمة</div><div style="font-size:18px;font-weight:900;color:#173f35;margin-top:5px">' + escapeHtml(event.emoji || "🌙") + ' ' + escapeHtml(nameAr) + '</div><div style="font-size:13px;line-height:1.7;color:#60716b;margin-top:5px">' + escapeHtml(descriptionAr) + '</div></div>'
...
const renderEmail = String.raw`async function renderEmail(env: Env, row: OutboxRow) {
  ...
  let builtIn = row.kind === "prayer" ? prayerDashboardEmail(data, "en") : builtInSystemEmail(row.kind, data, "en");
  let arabicSubject = row.kind === "prayer" ? prayerDashboardEmail(data, "ar").subject : builtInSystemEmail(row.kind, data, "ar").subject;
  ...
  if (!["verification", "manage", "admin_password_reset", "prayer"].includes(row.kind) && row.template_key) {
    ...
    if (template) {
      ...
      builtIn = { subject: applyTemplate(template.subject_en, values, false), html: applyTemplate(template.html_en, values, true), text: applyTemplate(template.text_en || builtIn.text, values, false) };
      if (template.subject_ar) arabicSubject = applyTemplate(template.subject_ar, values, false);
    }
  }

  if (arabicSubject && arabicSubject !== builtIn.subject) builtIn = { ...builtIn, subject: builtIn.subject + ' | ' + arabicSubject };
...
function replaceBlock(label, pattern, replacement) {
  if (!pattern.test(source)) throw new Error('Could not patch ' + label);
  source = source.replace(pattern, replacement);
}

replaceBlock('contentCard', /function contentCard\([\s\S]*?\n\}\n\nfunction enhancementHtml/, contentCard + '\n\nfunction enhancementHtml');
replaceBlock('enhancementHtml', /function enhancementHtml\([\s\S]*?\n\}\n\nfunction universalFooter/, enhancementHtml + '\n\nfunction universalFooter');
replaceBlock('universalFooter', /function universalFooter\([\s\S]*?\n\}\n\nfunction sponsorAttachment/, universalFooter + '\n\nfunction sponsorAttachment');
replaceBlock('renderEmail', /async function renderEmail\(env: Env, row: OutboxRow\) \{[\s\S]*?\n\}\n\nasync function sendResend/, renderEmail + '\n\nasync function sendResend');
...
console.log('Applied reliable mixed English/Arabic rendering and centered all email content.');
