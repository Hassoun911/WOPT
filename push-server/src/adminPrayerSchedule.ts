...
function to24(value: string, prayer: PrayerKey) {
  ...
  if (prayer !== "fajr" && hour < 12) hour += 12;
...
function parseMonthHtml(html: string, monthKey: string): PrayerTimes {
  ...
  for (const row of rows) {
    ...
    try {
      out[key] = {
        fajr: to24(cells[3], "fajr"),
        dhuhr: to24(cells[5], "dhuhr"),
        asr: to24(cells[6], "asr"),
        maghrib: to24(cells[7], "maghrib"),
        isha: to24(cells[8], "isha")
      };
    } catch {}
  }
  const expected = daysInMonth(year, month);
  if (Object.keys(out).length !== expected) throw new Error(`Expected ${expected} prayer rows for ${monthKey}, found ${Object.keys(out).length}`);
  return out;
}

async function fetchSourceMonth(env: Env, monthKey: string) {
  const base = env.ATHANPLUS_MONTHLY_URL || DEFAULT_SOURCE;
  const url = new URL(base);
  url.searchParams.set("date", `${monthKey}-01`);
  ...
  return { sourceUrl: url.toString(), prayerTimes: parseMonthHtml(html, monthKey) };
}
...
async function currentSchedule(env: Env): Promise<PrayerFile & Record<string, unknown>> {
  ...
  const data = await response.json() as PrayerFile & Record<string, unknown>;
  if (!data.prayer_times) throw new Error("Current schedule is missing prayer_times");
  return data;
}
...
function validateMonthData(monthKey: string, value: unknown): PrayerTimes {
  ...
  for (let day = 1; day <= expected; day += 1) {
    ...
    for (const prayer of PRAYERS) {
      const raw = typeof row[prayer] === "string" ? String(row[prayer]).trim() : "";
      if (!/^\d{2}:\d{2}$/.test(raw)) throw new Error(`Invalid ${prayer} time for ${key}`);
      normalized[prayer] = raw;
    }
    out[key] = normalized;
  }
  return out;
...
async function publishToGitHub(env: Env, schedule: Record<string, unknown>, monthKey: string) {
  const token = env.GITHUB_SCHEDULE_TOKEN;
  if (!token) throw new Error("GitHub schedule publishing is not configured yet");
  const owner = env.GITHUB_SCHEDULE_OWNER || DEFAULT_OWNER;
  const repo = env.GITHUB_SCHEDULE_REPO || DEFAULT_REPO;
  const path = env.GITHUB_SCHEDULE_PATH || DEFAULT_PATH;
  ...
  if (!currentJson.sha) throw new Error("GitHub file SHA is missing");
  const content = `${JSON.stringify(schedule, null, 2)}\n`;
  const update = await fetch(api, {
    method: "PUT",
    headers: { ...headers, "Content-Type": "application/json" },
    body: JSON.stringify({ message: `Update Windsor prayer schedule for ${monthKey}`, content: encodeBase64(content), sha: currentJson.sha, branch: "main" })
  });
  if (!update.ok) throw new Error(`GitHub publish failed: ${update.status} ${await update.text()}`);
  const result = await update.json() as { commit?: { sha?: string; html_url?: string } };
  return result.commit || null;
}

async function setSetting(env: Env, key: string, value: unknown, adminId?: number | null) {
  await env.DB.prepare(`INSERT INTO app_settings (setting_key,value_json,description,updated_by_admin_id) VALUES (?,?,?,?) ON CONFLICT(setting_key) DO UPDATE SET value_json=excluded.value_json,updated_by_admin_id=excluded.updated_by_admin_id,updated_at=CURRENT_TIMESTAMP`).bind(key, JSON.stringify(value), "Prayer schedule automation setting", adminId ?? null).run();
}

async function getSetting<T>(env: Env, key: string, fallback: T): Promise<T> {
  const row = await env.DB.prepare("SELECT value_json FROM app_settings WHERE setting_key=? LIMIT 1").bind(key).first<{ value_json: string }>();
...
async function requireOperator(request: Request, env: Env) {
  ...
  if (auth.admin.role !== "owner" && auth.admin.role !== "admin") return { admin: null, response: json({ error: "Owner or admin access required" }, 403, request, env) };
...
export async function handleAdminPrayerSchedule(request: Request, env: Env, url: URL) {
  ...
  try {
    if (request.method === "GET") {
      ...
      const monthTimes = Object.fromEntries(Object.entries(schedule.prayer_times).filter(([key]) => key.startsWith(`${month}-`)));
      const autoSync = await getSetting(env, "prayer_schedule_auto_sync", false);
      const lastAuto = await getSetting(env, "prayer_schedule_last_auto", null as null | Record<string, unknown>);
      return json({ ok: true, month, prayerTimes: monthTimes, sourceUrl: env.ATHANPLUS_MONTHLY_URL || DEFAULT_SOURCE, scheduleUrl: env.SCHEDULE_URL, autoSync, lastAuto, githubPublishingConfigured: Boolean(env.GITHUB_SCHEDULE_TOKEN) }, 200, request, env);
    }
    ...
    if (action === "preview") {
      const pulled = await fetchSourceMonth(env, month);
      return json({ ok: true, month, sourceUrl: pulled.sourceUrl, prayerTimes: pulled.prayerTimes }, 200, request, env);
    }
    if (action === "set_auto") {
      const enabled = body.enabled === true;
      await setSetting(env, "prayer_schedule_auto_sync", enabled, auth.admin.id);
      return json({ ok: true, autoSync: enabled }, 200, request, env);
    }
    if (action === "publish") {
      const monthData = body.prayerTimes ? validateMonthData(month, body.prayerTimes) : (await fetchSourceMonth(env, month)).prayerTimes;
      const schedule = await currentSchedule(env) as Record<string, unknown> & { prayer_times: PrayerTimes };
      schedule.prayer_times = { ...schedule.prayer_times, ...monthData };
      const metadata = schedule.metadata && typeof schedule.metadata === "object" ? schedule.metadata as Record<string, unknown> : {};
      schedule.metadata = { ...metadata, last_updated: new Date().toISOString(), monthly_source: env.ATHANPLUS_MONTHLY_URL || DEFAULT_SOURCE };
      const commit = await publishToGitHub(env, schedule, month);
      await setSetting(env, "prayer_schedule_last_publish", { month, at: new Date().toISOString(), commitSha: commit?.sha || null, source: "admin" }, auth.admin.id);
      return json({ ok: true, month, commit }, 200, request, env);
    }
    return json({ error: "Unknown action" }, 400, request, env);
  } catch (error) {
    return json({ error: error instanceof Error ? error.message : "Prayer schedule request failed" }, 500, request, env);
  }
}

export async function autoSyncPrayerSchedule(env: Env, scheduledTime: number) {
  const enabled = await getSetting(env, "prayer_schedule_auto_sync", false);
  if (!enabled || !env.GITHUB_SCHEDULE_TOKEN) return;
  const now = new Date(scheduledTime);
  const local = new Intl.DateTimeFormat("en-CA", { timeZone: "America/Toronto", year: "numeric", month: "2-digit", day: "2-digit" }).format(now);
  const already = await getSetting(env, "prayer_schedule_last_auto", null as null | { checkedDate?: string; month?: string; status?: string });
  if (already?.checkedDate === local) return;
  const [year, month, day] = local.split("-").map(Number);
  const targetDate = new Date(Date.UTC(year, month - 1 + (day >= 25 ? 1 : 0), 1));
  const targetMonth = `${targetDate.getUTCFullYear()}-${String(targetDate.getUTCMonth() + 1).padStart(2, "0")}`;
  try {
    const pulled = await fetchSourceMonth(env, targetMonth);
    const schedule = await currentSchedule(env) as Record<string, unknown> & { prayer_times: PrayerTimes };
    let changed = false;
    for (const [date, row] of Object.entries(pulled.prayerTimes)) {
      if (JSON.stringify(schedule.prayer_times[date]) !== JSON.stringify(row)) { changed = true; break; }
    }
    let commit: { sha?: string; html_url?: string } | null = null;
    if (changed) {
      schedule.prayer_times = { ...schedule.prayer_times, ...pulled.prayerTimes };
      const metadata = schedule.metadata && typeof schedule.metadata === "object" ? schedule.metadata as Record<string, unknown> : {};
      schedule.metadata = { ...metadata, last_updated: new Date(scheduledTime).toISOString(), monthly_source: env.ATHANPLUS_MONTHLY_URL || DEFAULT_SOURCE };
      commit = await publishToGitHub(env, schedule, targetMonth);
    }
    ...
    await setSetting(env, "prayer_schedule_last_auto", { checkedDate: local, month: targetMonth, status: "error", error: error instanceof Error ? error.message : String(error), at: new Date(scheduledTime).toISOString() });
  }
}
