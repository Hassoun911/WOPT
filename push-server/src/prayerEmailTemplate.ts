...
export function prayerDashboardEmail(data: Record<string, unknown>, locale: Locale): PrayerEmailRendered {
  ...
  const action = manageUrl
    ? `<tr><td align="center" style="padding:0 22px 22px"><a href="${esc(manageUrl)}" style="display:block;background:#0b654f;color:#fff;text-decoration:none;border-radius:14px;padding:14px 18px;font-size:13px;font-weight:900">${esc(t("⚙️ ⚙️ Manage prayer alerts 🔔 🔔", "⚙️ ⚙️ إدارة تنبيهات الصلاة 🔔 🔔"))}</a></td></tr>`
    : "";
...
</table><div style="max-width:600px;margin:12px auto 0;text-align:center;color:#9b988f;font-size:10px">Hassoun • ${esc(t("🕌 🕌 Stay connected to prayer • 🤲 Sadaqah Jariyah 🌙✨ • 🤲 Sadaqah Jariyah 🌙✨", "🕌 🕌 ابقَ متصلاً بالصلاة • 🤲 صدقة جارية 🌙✨ • 🤲 صدقة جارية 🌙✨"))}</div></td></tr></table></body></html>`;
