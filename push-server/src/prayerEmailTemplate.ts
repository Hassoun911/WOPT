...
export function prayerDashboardEmail(data: Record<string, unknown>, locale: Locale): PrayerEmailRendered {
  ...
  const subject = ar
    ...
    : kind === "twenty" ? `🕌🌙 ${prayer} in 20 minutes` : kind === "ten" ? `🕌✨ ${prayer} in 10 minutes` : `🕌🌙 It’s prayer time — ${prayer}`;
  const heroKicker = ar
  ...
  <tr><td style="padding:18px 22px;background:#fffdf8"><table role="presentation" width="100%"><tr><td width="58"><img src="${logo}" width="52" height="52" alt="Prayer" style="display:block;border-radius:15px;background:#003d33"></td><td style="padding-${ar ? "right" : "left"}:11px;text-align:${align}"><div style="font-size:12px;font-weight:900;letter-spacing:2px;color:#a0782c">${esc(t("PRAYER TIME", "وقت الصلاة"))}</div><div style="font-size:12px;color:#65756f;margin-top:2px">${esc(t("Prayer • Qur’an • Knowledge", "الصلاة • القرآن • المعرفة"))}</div></td><td style="text-align:${ar ? "left" : "right"};font-size:10px;color:#8e9189">${esc(location)}</td></tr></table></td></tr>
...
</table><div style="max-width:600px;margin:12px auto 0;text-align:center;color:#9b988f;font-size:10px">${esc(t("Stay connected to prayer", "ابقَ متصلاً بالصلاة"))}</div></td></tr></table></body></html>`;
