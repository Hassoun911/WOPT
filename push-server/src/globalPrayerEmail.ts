...
function methodFor(subscriber: Subscriber) {
  return subscriber.calculation_method ?? 3;
}

function locationKey(subscriber: Subscriber) {
  return [
    subscriber.latitude.toFixed(4),
    subscriber.longitude.toFixed(4),
    subscriber.timezone,
    methodFor(subscriber),
    subscriber.madhab
  ].join("|");
}
...
function sameMinute(leftMs: number, rightMs: number) {
...
async function subscribersWithPreferences(env: Env) {
  ...
  for (const row of results) {
    ...
    subscriber.preferences[row.prayer] = {
      email_twenty: row.email_twenty,
      email_ten: row.email_ten,
      email_athan: row.email_athan
    };
  }
...
async function cachedDay(env: Env, subscriber: Subscriber, dateKey: string) {
  return env.DB.prepare(
    `SELECT location_key, prayer_date, fajr, dhuhr, asr, maghrib, isha
     FROM location_prayer_cache WHERE location_key = ? AND prayer_date = ? LIMIT 1`
  ).bind(locationKey(subscriber), dateKey).first<CachedPrayerDay>();
}
...
async function fetchPrayerMonth(env: Env, subscriber: Subscriber, dateKey: string) {
  ...
  if (payload.code !== 200 || !Array.isArray(payload.data)) {
    throw new Error("Global prayer API returned an invalid calendar");
  }

  const key = locationKey(subscriber);
  const statements: D1PreparedStatement[] = [];
  for (const day of payload.data) {
    const prayerDate = gregorianDateKey(day.date?.gregorian?.date);
    ...
    statements.push(env.DB.prepare(
      `INSERT INTO location_prayer_cache (
         location_key, prayer_date, latitude, longitude, timezone,
         country_code, country_name, region, city, calculation_method, madhab,
         fajr, dhuhr, asr, maghrib, isha, source
       ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'aladhan')
       ON CONFLICT(location_key, prayer_date) DO UPDATE SET
...
async function ensureDay(env: Env, subscriber: Subscriber, dateKey: string) {
  let day = await cachedDay(env, subscriber, dateKey);
  if (day) return day;
  await fetchPrayerMonth(env, subscriber, dateKey);
  day = await cachedDay(env, subscriber, dateKey);
  if (!day) throw new Error(`Prayer calendar missing ${dateKey} after refresh`);
  return day;
}

async function claimPrayerEmail(
  env: Env,
  ...
  const result = await env.DB.prepare(
    `INSERT OR IGNORE INTO email_deliveries (
       event_id, subscriber_id, recipient_email, status, notification_kind,
       prayer, scheduled_for, template_key
     ) VALUES (?, ?, ?, 'pending', ?, ?, ?, 'prayer_alert')`
  ).bind(
    eventId,
    subscriber.id,
    subscriber.email,
...
async function queuePrayerEmail(
  env: Env,
  subscriber: Subscriber,
  day: CachedPrayerDay,
  prayer: PrayerKey,
  ...
  const delivery = await claimPrayerEmail(
    env,
    subscriber,
    eventId,
    kind,
  ...
  const data = {
    ...
    kind,
    prayer,
    prayerLabel: prayerLabel(prayer, subscriber.locale),
    prayerTime,
    prayerDate: day.prayer_date,
    locationLabel: locationLabel(subscriber),
...
async function evaluateDay(
  ...
  for (const prayer of PRAYERS) {
    const prefs = subscriber.preferences[prayer];
    if (!prefs) continue;
    const prayerTime = day[prayer];
    const prayerInstant = zonedLocalToInstant(day.prayer_date, prayerTime, subscriber.timezone);
...
export async function dispatchGlobalPrayerEmails(env: Env, scheduledTime: number) {
  const subscribers = await subscribersWithPreferences(env);
  let queued = 0;
  for (const subscriber of subscribers) {
    try {
      const now = new Date(scheduledTime);
      const todayKey = localDateKey(now, subscriber.timezone);
      const tomorrowKey = addDays(todayKey, 1);
      const [today, tomorrow] = await Promise.all([
        ensureDay(env, subscriber, todayKey),
        ensureDay(env, subscriber, tomorrowKey)
      ]);
      queued += await evaluateDay(env, subscriber, today, scheduledTime);
      // Pre-prayer reminders for a very early Fajr can fall on the previous date,
      // so tomorrow's prayer day must also be evaluated each minute.
      queued += await evaluateDay(env, subscriber, tomorrow, scheduledTime);
    } catch (error) {
      console.error("Global prayer email scheduling failed", {
